...
設備管理
    ...
    Sleep    1s
    ${device_response}    GET    url=http://192.168.11.26/api/Device/list    headers=&{headers}    expected_status=200
    ...
    ${post_response}    POST    url=http://192.168.11.26/api/Device/addDevNode    headers=&{headers}    json=${post_payload}    expected_status=200
...
情境管理
    [Documentation]    頁面列表是order by scriptId(以使用者角度來說就是亂排)
    Reload
    Wait For Element And Click It    id=device_management
    Wait For Elements State    id=next_page_url >>> css=body    visible
    ...
    Click With Options    id=basicOffcanvas_submit    delay=200ms
    Sleep    1s
    ${scenario_response}    GET    url=http://192.168.11.26/SerialScript?token=${token}    headers=&{headers}    expected_status=200
...
#    Click With Options    id=deleteConfirmModal_submit    delay=200ms
    ${put_response}    PUT    url=http://192.168.11.26/SerialScript/${script_id}    headers=${headers}    expected_status=200
    Assert Result 0    ${put_response}
    ${del_response}    DELETE    url=http://192.168.11.26/SerialScript/${script_id}    headers=${headers}    expected_status=200
    Assert Result 0    ${del_response}

自動化管理
    [Documentation]    已經搞不清楚列表的資料排序，元素指向?
    Reload
    Wait For Element And Click It    id=device_management
    Wait For Elements State    id=next_page_url >>> css=body    visible
    ...
    Click With Options    id=basicOffcanvas_submit    delay=200ms
    Sleep    1s
    ${trigger_response}    GET    url=http://192.168.11.26/api/Trigger/list?groupId=00000000-0000-0000-0000-000000000000&status=-1    headers=&{headers}    expected_status=200
    ...
    # 開關觸發、刪除
    ${set_response}    POST    url=http://192.168.11.26/api/Trigger/setEnable?triggerid=${trigger_id}&enable=0    headers=&{headers}    expected_status=200
    Assert Result 0    ${set_response}
    ${del_response}    DELETE    url=http://192.168.11.26/api/Trigger/del?triggerid=${trigger_id}    headers=&{headers}    expected_status=200
    Assert Result 0    ${del_response}
...
需量管理
    ...
    Select Options By    id=demand_demand_target_device    value    13F-VRF-AC-B5-35-F
    ...
    Click With Options    id=basicOffcanvas_submit    delay=200ms
    Sleep    1s
    ${demand_response}    GET    url=http://192.168.11.26/demandControl/config/?token=${token}    headers=&{headers}    expected_status=200
    ...
    # 開關觸發、刪除
    ${set_response}    POST    url=http://192.168.11.26/api/Trigger/setEnable?triggerid=${demand_id}&enable=0    headers=&{headers}    expected_status=200
    Assert Result 0    ${set_response}
    ${del_response}    DELETE    url=http://192.168.11.26/demandControl/config/${demand_id}    headers=&{headers}    expected_status=200
    Assert Result 0    ${del_response}