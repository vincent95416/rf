...
頁面標籤、widget新增
    ...
    ${page_response}    POST    url=http://192.168.11.26/TreeData/treedata/?token=${token}    headers=&{headers}    json=&{newpage_payload}    expected_status=200
    ...
    ${tag_response}    POST    url=http://192.168.11.26/TreeData/treedata/?token=${token}    headers=&{headers}    json=&{newtag_payload}    expected_status=200
    ...
    ${secondtag_response}    POST    url=http://192.168.11.26/TreeData/treedata/?token=${token}    headers=&{headers}    json=&{secondtag_payload}    expected_status=200
...
#    Log    ${trigger_id}
    &{headers}    Create Dictionary    Content-Type=application/json    Authorization=${token}
    Sleep    5s
    ${setvalue_response}    GET    url=http://192.168.11.26/setValue?token=${token}&devid=13F-VRF-AC-B5-35-F&node=TempSet&val=${set_temp}    headers=&{headers}    expected_status=200
    ...
    ${delpage_response}    DELETE    url=http://192.168.11.26/TreeData/treedata/${menu_id}    headers=&{headers}    expected_status=200
