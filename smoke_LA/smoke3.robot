...
新增平面圖
    ...
    Select Options By    id=iframe_map >>> id=iframe_map_widget >>> id=device_model_select    value    1
    ...
    ${nowrap1}    Get text    id=iframe_map >>> xpath=/html/body/div[2]/div[2]/div/div[2]/div[2]/div[3]/span[2]
    Should Contain    ${nowrap1}    一氧化碳
    Sleep    1s
...
編輯平面圖
    ...
    Wait For Element And Click It    id=iframe_map >>> id=add_widget_btn
    Wait For Elements State    id=iframe_map >>> id=iframe_map_widget >>> css=body    visible    15s
    Select Options By    id=iframe_map >>> id=iframe_map_widget >>> id=device_model_select    value    2
    ...
    Click With Options    id=iframe_map >>> id=iframe_map_widget >>> id=add_btn    delay=200ms
    Sleep    1s
    ${icon2}    Get Element    id=iframe_map >>> css=span[databind='${device2}NODEco']
    Hover    ${icon2}
    Wait For Elements State    id=iframe_map >>> .map_widget_hover    visible
    ${nowrap5}    Get text    id=iframe_map >>> xpath=/html/body/div[2]/div[2]/div/div[2]/div[2]/div[5]
    Should Contain    ${nowrap5}    Fan Speed
    Sleep    1s
    ...
    ${outside_nowrap5}    Get Text    id=next_page_url >>> id=${WIDGET_ID}_MAP >>> xpath=/html/body/div[2]/div[2]/div/div[2]/div[2]/div[5]/span[1]
    Should Contain    ${outside_nowrap5}    ${nowrap5}
